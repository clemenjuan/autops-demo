...
from src.eventsat.rl_obs_encoder import _DEFAULT_JETSON_CAPACITY_MB
...
class SubsymbolicEventSat(Representation):
    ...
    def __init__(self, config: Dict[str, Any] | None = None) -> None:
        ...
        # RL contract for this scenario (mode list, action dims, obs encoder).
        # Defaults to EventSat; the runner injects ``scenario`` (e.g. "ssa") so
        # SSA gets its 8-mode action space + SSA observation encoder. Same class,
        # no per-scenario subclass.
        from src.rl.space_adapters import get_rl_spec

        spec = get_rl_spec(str(self.config.get("scenario", "eventsat"))) or get_rl_spec("eventsat")
        self._mode_list = spec.mode_list
        self._base_action_dims = spec.action_dims
        self._obs_encoder = spec.obs_encoder
        ...
        if not self._action_dims:
            self._action_dims = [1]
        self._obs_dim = spec.obs_dim * len(self._observe_ids) + self._message_dim
        if self._obs_dim <= 0:
            self._obs_dim = 1
