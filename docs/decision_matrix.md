...
**Ground-segment information model (adopted 2026-06-12; impact verified 2026-06-13).** Two construct decisions, found during the CG investigation and applied to all ground paradigms (AH-ground/AG/CG): (1) **planners receive the TRUE inter-pass gap** from the environment's pass table — pass prediction is deterministic ground-segment capability (orbit propagation × station geometry; Sellmaier et al. 2022 §16.4), so the previous one-orbit default (93 steps) was an unjustified myopia that capped every ground schedule inside 92–764-step real gaps. AG/AH plan the gap from current-pass end to next-pass start; **CG plans the *following* gap** (next-pass end → subsequent-pass start), because its schedule uplinks one pass later — the team plans against the contact window its schedule will actually cover. (2) **Uplink requires an established link**: CG promotes its staged schedule only when the spacecraft actually reaches communication mode during the pass (the runner's resolved-mode signal, same gate as telemetry downlink); a pass shorter than the ADCS settling time transfers nothing in either direction.

**Verified-neutral on the A1 anchor (2026-06-13) — and why that is the right result.** Both fixes are correct but change the A1 measured gradient by **zero**: AG episode-0 utility is bit-identical pre/post (3.591369047619048), and the 100-ep means are unchanged (CG 2.99 < AG 3.62 < AO 3.86 < AH 4.14). Reason: in A1, neither defect was the binding constraint. The schedule's *productive* content is gated by battery (>0.60 SoC), OBC fill (<80 %), and the 27 MB/day downlink budget — these saturate after the same handful of observations regardless of horizon length, so a longer planning gap is filled with trailing charging the satellite would do anyway (an earlier counterfactual that AG would *triple* its observations was **wrong** — retracted). Link-gating likewise never triggered because A1 Ottobrunn passes are long enough to establish comms. **This is a robustness result, not a null result:** the headline paradigm gradient is invariant to two plausible ground-segment modelling choices, which strengthens the EventSat claim. The fixes still matter for correctness and will bite where they should — short-pass or storage-rich regimes (where horizon binds) and degraded-link scenarios (T-axis MC.02/AU.06). Note: the earlier "observation-free CG" exclusion was already falsified separately — pre-fix CG measured U=3.02 with ~2 obs/gap (its own `max_observations_per_gap` cap), planning-time SoC always ≈0.96–0.99 so the cognitive parameterization never engaged.
...
- **Sample size, mathematically determined (not asserted):** for a two-sided comparison at significance \(\alpha\) (Bonferroni: \(\alpha/m\) for a family of \(m\) tests) with power \(1-\beta\), the required number of **paired** episodes — architectures share launch-lottery seeds, with pairing correlation \(\rho\) — is
  \[ n_{pairs} = \frac{(z_{1-\alpha/2m} + z_{1-\beta})^2 \cdot 2(1-\rho)}{d^2} \cdot \frac{1}{0.955} \]
  (the 0.955 divisor is the Wilcoxon ARE vs the t-test under normality; unpaired comparison instead needs \(n = 2(z_{1-\alpha/2m}+z_{1-\beta})^2/d^2\) per group). **Measured pilot inputs (EventSat anchor, symbolic AO/AH, 100 episodes, 2026-06-11):** \(\rho = 0.79\), \(\sigma_U \approx 0.17\). Consequences: detecting \(d=0.5\) at \(\alpha=0.05\), power 0.8 needs **14 paired episodes** (vs 63/group unpaired); \(d=0.3\) needs 38; the pre-registered 100 episodes give power ≥ 0.8 down to \(d \approx 0.25\) even under Bonferroni \(m=10\). \(\rho\) and \(\sigma_U\) are re-estimated per substrate family (LLM-cell variance is larger at pilot episode lengths) and per SSP; the formula, not the numbers, is the pre-registered rule.
