...
- The **agentic** ground prompt (`AGENTIC_SCHEDULE_SYSTEM_PROMPT`) is deliberately **decisive** ("you already have the telemetry; tools only validate/score a candidate; one check is usually enough; think briefly then act"). qwen3.6 is a *thinking* model and an open-ended agentic prompt makes it deliberate the whole loop in its `<thinking>` trace → unbounded spiral that blows the 300 s wall-clock timeout (the direct single-shot prompt terminates ~29.5 s). Don't loosen it back to "call tools multiple times / verify assumptions" without re-checking it still terminates. (`think:false` "fixes" the spiral but makes the model emit malformed JSON — rejected.) Two reinforcing levers also bound per-decision latency (so a fresh-telemetry decision doesn't overrun the contact window): the *echo* tools (battery/pass/pipeline state the planner already receives) are **folded into the planning prompt** (`DERIVED` block) instead of fetched via an LLM round-trip — only the **what-if** tools (`check_constraints`, `evaluate_plan`; +`recall_history` for the per-step core, not the scheduler — see `SCHEDULE_TOOL_NAMES`) are advertised — and `max_agentic_steps` default is **3** (worst case ~4 LLM calls). Don't re-register the echo tools or raise the budget without re-checking latency.
