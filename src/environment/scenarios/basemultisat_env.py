...
standard multi-satellite contract:
  ...
  * ``StepResult.rewards`` keyed **per satellite** (``{"sat_0": r0, ...}``), so
    ...
    (see ``src/environment/rewards.py``) — a future scenario-specific reward
...
class BaseMultiSatEnvironment(SatelliteEnvironment):
    ...
    def _aggregate_info(per_satellite_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sum_keys = (
            ...
            "step_downlinked_mb", "total_pass_duration_s",
        )
        mean_keys = ("battery_soc", "prev_battery_soc")
        any_keys = ("in_sunlight", "ground_pass_active", "forced", "in_transition")
