...
- Jetson-based onboard cores (subsymbolic/hybrid onboard, AO/AH) keep the Jetson powered → `power.onboard_compute_w` (≈7 W) **added** to modes where it'd otherwise be off (charging/communication/observe/safe), but NOT to the Jetson-compute modes `power.jetson_active_modes` (compress/detect/send already include the working Jetson — no double-count). Symbolic onboard runs on the OBC (sub-watt) → no overhead; ground paradigms (AG/CG) → no overhead. Wired via `config.onboard_uses_jetson` → `env.onboard_compute_active`
