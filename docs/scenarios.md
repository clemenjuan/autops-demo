...
- Power budget (solar charging vs. consumption duty cycle; 84 Wh battery, 24 W solar). Onboard autonomy (AO/AH) adds a continuous Jetson compute draw (`power.onboard_compute_w`) on top of per-mode consumption; ground paradigms (AG/CG) have no such overhead.
