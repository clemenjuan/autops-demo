...
class TestCmdTrainPPO:
    def test_train_ppo_calls_ppo_trainer(self, tmp_path: Path) -> None:
        ...
        with patch("src.core.behaviour.training_pipeline.PPOTrainer", return_value=mock_trainer):
            args = MagicMock()
            args.config = str(config_path)
            args.timesteps = 1000
    ...
    def test_train_ppo_missing_torch_exits(self, tmp_path: Path) -> None:
        ...
        with patch.dict("sys.modules", {"src.core.behaviour.training_pipeline": None}):
            args = MagicMock()
            args.config = str(config_path)
            args.timesteps = None
            args.source_dir = None
...
class TestCmdTrainPromptOptimized:
    def test_train_prompt_optimized_calls_optimizer(self, tmp_path: Path) -> None:
        ...
        with patch("src.core.behaviour.prompt_optimizer.PromptOptimizer", return_value=mock_optimizer):
            args = MagicMock()
            args.config = str(config_path)
            args.timesteps = None
